...
CREATE TABLE Caixa(
    ...
    idEncomenda INTEGER        CONSTRAINT fk_caixa_encomenda REFERENCES Encomenda (idEncomenda)         ON DELETE CASCADE         
                                                                                                        ON UPDATE CASCADE,

    volume INTEGER           CONSTRAINT caixa_volume_positive CHECK (volume > 0)
                             CONSTRAINT caixa_volume_nn NOT NULL,
    peso INTEGER             CONSTRAINT caixa_peso_positive CHECK (peso > 0)
                             CONSTRAINT caixa_volume_nn NOT NULL
);
